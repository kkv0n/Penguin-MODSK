...
void DECOMP_MM_TrackSelect_Video_Draw(RECT *r, struct MainMenu_LevelRow *selectMenu, int trackIndex, int param_4, u_short param_5)
{
  ...
  // FORCE STATIC IMAGE MODE - ALWAYS
  // This completely disables video playback
  D230.trackSel_videoStateCurr = 1;
  
  selectMenu = &selectMenu[trackIndex];
  videoID = selectMenu->videoID;

  // Always use static images - skip video code entirely
#ifndef REBUILD_PS1
  // If we have allocated video memory, free it
  if (D230.trackSel_video_boolAllocated != 0)
  {
    MM_Video_StopStream();
    MM_Video_ClearMem();
