...
void DECOMP_UI_DrawSlideMeter(short posX, short posY, struct Driver* driver)
{
	...
	const int xOffset = gGT->numPlyrCurrGame == 1 ? 2 : 0;
	const int barWidth = gGT->numPlyrCurrGame == 1 ? 39 :  WIDE_34(49);
	int barHeight = gGT->numPlyrCurrGame == 1 ? 10 : (gGT->numPlyrCurrGame > 2 ? 3 : 7);
	posX += gGT->numPlyrCurrGame == 1 ? xOffset : 0;
	#else
