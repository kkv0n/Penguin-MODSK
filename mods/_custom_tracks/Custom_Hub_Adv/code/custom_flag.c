		...
		for (row = 0; row < 10; row++)
		{
			...
			for (/**/; j < 3; posR++, posL++, j++, i++)
			{
					...
					((dimensions - posR[0] & dimensions - posR[1] & dimensions - posL[0] & dimensions - posL[1] & screenlimit) == 0))
				{
					...
					RaceFlag_SetRGB(p, ((boolDark) ? BGR_TO_RGB(CTR_FontColor(raceflag_dark_color)) :
					BGR_TO_RGB(CTR_FontColor(raceflag_light_color))), colorR, false);
					...
					*(int*)&p->r2 = *(int*)&p->r0;
					...
					RaceFlag_SetRGB(p, ((boolDark) ? BGR_TO_RGB(CTR_FontColor(raceflag_dark_color)) :
					BGR_TO_RGB(CTR_FontColor(raceflag_light_color))), colorL, true);
					*(int*)&p->r3 = *(int*)&p->r1;
