...
int LOAD_TenStages(struct GameTracker* gGT, int loadingStage, struct BigHeader* bigfile)
{
	...
	CTRBIGFILE VRAM_F[] = { TRACK_1_RAM, TRACK_2_RAM, TRACK_3_RAM, TRACK_4_RAM };
	CTRBIGFILE LEV_F[] = { TRACK_1_LEV, TRACK_2_LEV, TRACK_3_LEV, TRACK_4_LEV };
	d_vram = VRAM_F[selected_track];
	d_track = LEV_F[selected_track];
   ...
   */
   ...
   // impact any intended loading screens
	...
	case 1:
	{
		...
		//  always load time trial overlay (for end race menu) 
		ovrRegion1 = 3;
...
// loop through models
		...
		if (gGT->levelID == track_id)
		{
			// adds VRAM to loading queue
			LOAD_AppendQueue(sdata->ptrBigfile1, 3, d_vram, 0, 0);
			// adds LEV to loading queue
			LOAD_AppendQueue(sdata->ptrBigfile1, 2, d_track, 0x80300000, &LOAD_Callback_LEV);
