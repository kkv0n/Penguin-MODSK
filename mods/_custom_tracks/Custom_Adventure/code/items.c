...
void VehPhysGeneral_SetHeldItem(struct Driver* d) {
	...
	if ((gGT->gameMode1 & BATTLE_MODE) == 0)
	{
		...
		if ((gGT->gameMode1 & CRYSTAL_CHALLENGE) == 0)
		{
			...
			int mode = 2; //keep 1 case for all players
			...
			switch(mode)
			{
				...
				case 2:
					...
					// boss race, last place
						...
						Itemset1:
						// 1st Itemset
						itemSet = ITEMSET_Race1; //assign itemset1 (actually it is changed for a custom one later)
				...
				case 3:
					...
					// if first place
					if (d->driverRank == 0) goto Itemset1;
	...
	switch(itemSet)
	{
		...
		case ITEMSET_BossRace:
			d->heldItemID = item_set[(rng * numWeapons[itemSet]) / 0xc8];
			break;
