...
extern unsigned char raceflag_dark_color;
extern unsigned char raceflag_light_color;
...
force_inline void RaceFlag_SetRGB(POLY_G4 *p, u_int color, u_char pixBrightness, u_char left)
{
	...
	u_int rgb[3];
	rgb[0] = (r * pixBrightness) >> 8;
	rgb[1] = (g * pixBrightness) >> 8;
	rgb[2] = (b * pixBrightness) >> 8;
		...
		for (row = 0; row < 10; row++)
		{
			...
			for (/**/; j < 3; posR++, posL++, j++, i++)
			{
					...
					((dimensions - posR[0] & dimensions - posR[1] & dimensions - posL[0] & dimensions - posL[1] & screenlimit) == 0))
				{
					...
					//set dark tiles from the custom race flag
					u_int CurrTile = CUSTOM_RACE_FLAG(raceflag_dark_color);
					
					//set light tiles from the custom race flag
					if (!boolDark)
					{
					 CurrTile = CUSTOM_RACE_FLAG(raceflag_light_color);
					}

					u_char colorR = RaceFlag_CalculateBrightness(lightR, boolDark);
					RaceFlag_SetRGB(p, CurrTile, colorR, false);
					*(int*)&p->r2 = *(int*)&p->r0;


					u_char colorL = RaceFlag_CalculateBrightness(lightL, boolDark);
					RaceFlag_SetRGB(p, CurrTile, colorL, true);
					*(int*)&p->r3 = *(int*)&p->r1;
