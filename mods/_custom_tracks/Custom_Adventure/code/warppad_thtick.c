...
void  AH_WarpPad_ThTick(struct Thread* t)
{
	...
	if (
		...
		///???
		//why not just use the level enum??
...
#define InstArr0 warppadInst
	...
	if (warppadObj->framesWarping == 1)
	{
		...
		if ((levelID < GEM_STONE_VALLEY) || (levelID >= 100))
		{
			...
			else
			{
				...
				for (i = 0; i < 7; i++)
				{
					...
					sdata->kartSpawnOrderArray[randKartSpawn[rng2]] = 1;
	...
	void custom_adventure(u_char levelID, bool boss);
	custom_adventure(levelID, false);
}
