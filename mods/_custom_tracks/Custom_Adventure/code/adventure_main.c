...
void adventure_main()
{
  ...
  if (gGT->trafficLightsTimer >= 3490 && gGT->trafficLightsTimer <= 3500
                 && gGT->levelID < GEM_STONE_VALLEY)
	{
		...
		inst = (current_track >= N_SANITY_BOSS) ? UI_INSTANCE_BirthWithThread(STATIC_KEY,(int)UI_ThTick_Reward,0xf,1,0,/*sdata->s_key1*/0) :
		UI_INSTANCE_BirthWithThread(0x61, (int)UI_ThTick_Reward,0xe,1,0,/*sdata->s_relic1*/0);
		
		inst->matrix.t[0] = (current_track >= N_SANITY_BOSS) ? -400 : -206;
		inst->matrix.t[1] =  (current_track >= N_SANITY_BOSS) ? 152 : 86;
