...
// To do: add a header
void  AA_EndEvent_DisplayTimeAA_EndEvent_DisplayTime(u_short, short);
	...
	if (playerThread != 0)
	{
		...
		do
		{
			...
			if ((gameMode1 & BATTLE_MODE) == 0)
			{
				...
				if ((playerStruct->actionsFlagSet & 0x2000000) == 0)
				{
					 UI_DrawLapCount(hudStructPtr[1].x, hudStructPtr[1].y, (u_int)hudStructPtr[1].scale, playerStruct);
				}
			}
...
// not rewritten yet
			...
			if ((partTimeVariable5 & 0x4020020) == 0)
			{
				...
				if (!ghostMode && gGT->trafficLightsTimer < 1)
			...
			else if ((partTimeVariable5 & 0x200020) == 0x200020)
			{
				...
				if (!ghostMode)
					UI_DrawPosSuffix(hudStructPtr[5].x, hudStructPtr[5].y, playerStruct, (short)(partTimeVariable5 << 2));
