...
void StateZero()
{
	...
	LOAD_XnfFile("\\LOD.BIN;1", (void*)(MEMPACK_SIZE ADD_PSX_ADDRESS), &ramSize);
	LOAD_XnfFile("\\FUNCS.BIN;1", (void*)(0x700fd8 ADD_PSX_ADDRESS), &ramSize);
	...
	*(u_int*)&gGT->battleSetup.teamOfEachPlayer = 0x3020100;
	...
	LOAD_LangFile((int)sdata->ptrBigfile1, 1); //lol...
