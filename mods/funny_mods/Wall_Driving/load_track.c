...
// loop through models
	...
	case 7:
	{
		...
		//swap wall quadflag with ground quadflag
		for (int i = 0; i < gGT->level1->ptr_mesh_info->numQuadBlock; i++)
		{
			//if this quadblock have wall flag
			if ((gGT->level1->ptr_mesh_info->ptrQuadBlockArray[i].quadFlags & (1 << 13)) != 0)
			{
				//give speed impact to this quadblock
				//if the level is sewer then avoid giving all quads -127 because it will enable the turbo in unintended zones
				gGT->level1->ptr_mesh_info->ptrQuadBlockArray[i].speedImpact = (gGT->levelID == SEWER_SPEEDWAY) ? -126 : -127;

				//clear wall quadflag
				gGT->level1->ptr_mesh_info->ptrQuadBlockArray[i].quadFlags &= ~(1 << 13);

				//enable ground quadflag in this quadblock
				gGT->level1->ptr_mesh_info->ptrQuadBlockArray[i].quadFlags |= (1 << 12);
			}
		}
