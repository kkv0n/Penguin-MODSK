...
void ServerState_Tick()
{
	...
	for (int r = 0; r < 16; r++)
	{
		...
		if (!ri->boolEndAll)
		{
			...
			{
				printf("1 player detected, finishing\n");
				ri->boolEndAll = 1;
			}

			if (ri->finishedCount == ri->requiredPlayersToFinish && ri->requiredPlayersToFinish != 0) { //if required players to finish already ended the race
				printf("used default counter\n");
				...
				if (ri->boolEndAll) {
					PrintPrefix(r + 1);
					printf("Timer Started, resetting room soon\n");
		...
		{
			if (((clock() - ri->endTime) / CLOCKS_PER_SEC_FIX) >= ri->timeLimit)
			{
				PrintPrefix(r + 1);
